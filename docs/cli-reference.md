...
- stdin JSON includes `session_id`, `cwd`, `hook_event_name: "PreToolUse"`, `tool_name`, and a `tool_input` object whose shape depends on `tool_name`:
...
- stdin JSON is near-identical to Claude Code's — same `hook_event_name: "PreToolUse"` value, same `session_id`/`tool_name`/`tool_input.command` fields — but additionally includes `turn_id` and `tool_use_id`, which Claude Code's own payload never sends (Claude Code sends a `prompt_id` instead). **This is the actual discriminator `cli/cmd/hook.go` uses to tell the two apart** — `hook_event_name` alone can't, since Codex deliberately reuses Claude Code's exact value (OpenAI built Codex's hook contract to be compatible with existing Claude Code hook scripts).
