...
**The one exception, added 2026-08: `--hook-json` (embedding-host mode).** Everything above describes a hook invoked by an agent running at a terminal. An application that *embeds* an agent SDK (a desktop app, an IDE) has no terminal at all, but does have a human sitting in front of it and its own permission dialog — so for it, the no-TTY deny above turns most of the shipped policy into a silent hard block the user can't answer. With `--hook-json`, `runHook` writes one JSON object on stdout (`cli/adapter/hook/hostresponse.go`) and hands a `Prompt`-tier decision to that host as `permissionDecision: "ask"` instead of resolving it itself. Three properties make this safe to add without weakening the terminal path: it is an explicit flag that `damping init` never writes into any agent's hook config; it never emits `permissionDecision: "allow"` (that value means "skip the host's own permission flow", which is auto-approval, not a guardrail's job — a plain allow carries no `hookSpecificOutput` at all); and a deny is still a deny, exit code 2 included. The reason it is a flag rather than an automatic no-TTY fallback is documented in `hostresponse.go` and `docs/threat-model.md` §6.1 — the automatic version was prototyped and parked because Claude Code's "auto" mode treats `ask` as allow and Cursor's `ask` has shipped broken, i.e. both fail unsafe. See `docs/cli-reference.md` §11.1 for the response schema.

This dispatch is keyed primarily on the payload's own event-name field — `beforeShellExecution` only ever means Cursor — but `hook_event_name: "PreToolUse"` alone is ambiguous: Codex deliberately reuses Claude Code's exact value (OpenAI built Codex's hook contract to be compatible with existing Claude Code hook scripts). The two are told apart by `turn_id` alone, which Codex's real payload includes and Claude Code's does not — **not** `tool_use_id`, which a directly captured real Claude Code payload turned out to send as well (a live misattribution bug, fixed 2026-07-07: every Claude-Code-only user's audit entries were being labelled "codex") — see `docs/cli-reference.md` §11. A payload from an agent Damping still doesn't recognize at all fails open (same as any other unrecognized-but-valid input), but it still writes a `degraded` audit record via the same path as malformed JSON, rather than passing through with no trace at all — this was a real gap until it was closed (a `default:` branch that returned silently, worse than the malformed-JSON path, since even that already logged degraded).

**Non-Bash dispatch (2026-07 expansion)**: within a `"PreToolUse"` payload, `runHook` further branches on `tool_name`. `"Bash"` takes the shell-AST path described above (`hookadapter.EvaluateCommand` → `shell.Analyze`). `"Write"`/`"Edit"`/`"MultiEdit"` instead call `hookadapter.FactsFromToolWrite` directly — no shell parsing involved, since a file write isn't shell text — producing a `policy.Facts` with `ActionType: event.ActionConfigWrite` that's evaluated the same way (`engine.Evaluate`, wrapped in its own panic-recovering `evaluateFactsRecovering`, mirroring `evaluateCommandRecovering`'s existing fail-open-and-degraded design for adversarial input). A `tool_name` starting with `mcp__` (the agent's `mcp__<server>__<tool>` namespacing) takes a third path, `hookadapter.FactsFromMCPToolCall` — see the MCP note below. Any other `tool_name` (`Read`, `Grep`, ...) is a no-op — nothing in Damping's V1 policy scope judges it. Only Claude Code's own hook registration actually reaches this branch in practice: `cli/adapter/agent/claude_code.go`'s matcher covers `"Bash|Write|Edit|MultiEdit"`, but Cursor has no pre-write hook and Codex's `PreToolUse` never fires for these tool names — see `docs/cli-reference.md` §11's capability matrix.
...
**MCP tool calls that `mcp wrap` structurally cannot see (2026-08).** `wrap` is a stdio proxy, so it only governs servers the agent launches as a subprocess; an HTTP/SSE MCP server is out of its reach entirely. The agent's own `PreToolUse` hook does still fire for those calls, so `cli/adapter/hook.FactsFromMCPToolCall` builds the *same* `policy.Facts` shape `facts.go` builds here (same `Channel`, `ActionType`, `Command`, and `"<tool name> <args json>"` `Raw`), from the hook payload instead of from a live tool list. One rule written once therefore behaves identically on both paths, and an `[A]` always-allow pattern persisted on either matches the same call arriving through the other. The unavoidable difference is annotations: `wrap` reads `ToolAnnotations` off the server itself, while the hook contract carries none — which is why `mcp.destructive_tool_name` (a whole-word read of the tool's own name) exists as the annotation-free fallback, and why a host that already holds the server's tool list can pass the annotations through as a Damping-specific `tool_annotations` extension field.
