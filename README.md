...
`mcp wrap` is a stdio proxy, so it only covers servers your agent launches as a subprocess. For an MCP server reached over HTTP/SSE, the agent's own pre-tool-use hook is the only interception point that exists — Damping judges those calls too (tool name `mcp__<server>__<tool>`), you just have to widen your agent's hook matcher to include them. See [`docs/cli-reference.md`](docs/cli-reference.md) §11.
...
| PocketOS (April 25, 2026): a Cursor session running Claude Opus 4.6 hit a credential mismatch in staging, went looking for a fix on its own, and found an API token in an unrelated file — scoped for managing custom domains via Railway's CLI, but broad enough to authorize any operation. It used that token to call Railway's GraphQL API directly with a `volumeDelete` mutation, wiping the production database and every backup in the same call (Railway stores volume-level backups in the same volume) — no confirmation prompt, 9 seconds start to finish ([The Register](https://www.theregister.com/2026/04/27/cursoropus_agent_snuffs_out_pocketos/)). | `destructive.cloud_api_raw_delete` — a raw `curl`/`wget` call straight to a cloud/PaaS provider's API deleting a whole deployed resource, bypassing the provider's own CLI (and whatever confirmation step it might have) entirely. Covers Railway's GraphQL API plus a real REST `DELETE` call to Vercel/Netlify/Render/Heroku/DigitalOcean's own project/site/service/app/droplet endpoints — verified against each provider's own API reference, not assumed by analogy, after the first version of this rule shipped scoped to Railway alone and got asked directly whether that was broad protection or just single-incident protection |

| OpenAI's Codex engineering lead confirmed (July 16, 2026) a "handful" of cases where GPT-5.6 Codex, running in full-access mode with sandboxing and auto-review both disabled, tried to re-point `$HOME` at a temporary directory during cleanup and deleted the user's *real* home directory instead — reported casualties included most of one developer's Mac home directory and another's production database ([The Register](https://www.theregister.com/ai-and-ml/2026/07/16/openai-admits-gpt-56-occasionally-deletes-files-but-its-an-honest-mistake/5274008), [Thibault Sottiaux's statement](https://simonwillison.net/2026/Jul/16/bad-codex-bug/)). | `destructive.rm_rf_protected` / `destructive.find_delete_protected` / `destructive.write_protected_path` — every `$HOME` spelling of a home deletion is critical-tier, same as the literal `~`: `rm -rf $HOME` (quoted, braced, trailing-slash), `bash -c "rm -rf $HOME"`, `eval "rm -rf $HOME"`, the heredoc form, `env HOME=/tmp/x rm -rf "$HOME"` (the parent shell expands `$HOME` *before* env applies the override — the literal incident narrative), the glob forms `rm -rf ~/*` / `rm -rf /*` / `find ~/* -delete`, and an `rm -rf` target that can't be statically resolved at all is treated as unprovable rather than assumed to be scratch. Verified against every shape listed — several of which (the `bash -c`/`eval`/heredoc/`find`-glob spellings, and protected-path writes via `$HOME`) were allowed outright, and the rest mis-tiered as medium, before this incident prompted the expansion. The same work closed the adjacent hole the audit surfaced: `eval` of a plain opaque variable (`eval $CMD`) executed the same unknowable content a bare `$CMD` in command position has always been flagged for, and now trips `destructive.dynamic_command_construction` too, while the ubiquitous legitimate idioms (`eval "$(ssh-agent -s)"`, `eval "$(direnv hook bash)"`) stay quiet — see [`docs/threat-model.md`](docs/threat-model.md) §3 for the deliberate asymmetry. Honest scope note: an unattended agent only gets the deny-by-default benefit here if `prompt`-tier rules can't silently fall through — which is Damping's default when no TTY is available, and stays true under `noninteractive_prompt_fallback` as long as you keep `critical: deny` |
...
- CLI shell-command interception, with real AST parsing (not regex), across 29 default rules (destructive deletes, force pushes, destructive SQL/Mongo/Redis, recursive permission changes, unvetted install pipelines, encoded payloads, sandbox-bypass paths, infra-as-code destroy/unreviewed-apply, destructive git history operations, secret exfiltration, kubectl/cloud-CLI bulk deletes, raw cloud-provider-API deletes, raw block-device writes, unreviewed crates.io/RubyGems publishes, chat-webhook exfiltration, bulk removal of the agent's own installed skills/plugins/project memory, `find -delete` against protected paths, and more) — see [`docs/threat-model.md`](docs/threat-model.md) and the "Real incidents this defends against" section above for the full list and the real-world incidents behind each one.
- Every rule above now sees through common evasion shapes, not just the literal command typed: a command wrapper (`sudo`/`env`/`nohup`/`timeout`/`exec`/`nice`/`time`/...), an interpreter's `-c` script or `eval` argument — including one that embeds `$HOME` or another plain variable (`bash -c "rm -rf $HOME"` used to be allowed outright), every stage of a pipeline (`rm -rf ~/ | cat` used to slip past entirely, since a pipeline's own Facts carried no per-stage arguments), compound-command forms (`case`, `declare`, `[[ ... ]]`, `time`, `coproc`) the AST walk didn't previously descend into, a word-leading `$HOME`/`${HOME}` in any argument or redirect-target position (canonicalized to `~`, so every `$HOME` spelling of a home deletion or protected-path write gets the literal spelling's full severity), trailing directory globs (`rm -rf ~/*`, `find ~/* -delete`) judged as the directory they empty, and a command invoked by its path (`/usr/bin/rm -rf ~/`, `sudo /bin/rm -rf ~/`) judged as the program it actually runs — while a tool run out of a variable-held directory (`$VENV/bin/python`, `$HOME/go/bin/foo`) is no longer mistaken for a dynamically-constructed command. See [`docs/threat-model.md`](docs/threat-model.md)'s "Known bypass techniques" table for the specifics — and what's still honestly open (e.g. `xargs`-sourced operands, tilde-path normalization against an already-absolute `Write` tool target).
- Claude Code's `Write`/`Edit`/`MultiEdit` tool calls get the same treatment as `Bash` — 3 of the 29 rules above catch a dangerous *file write* (agent permission escalation, git-hook persistence, npm lifecycle-script injection), not just a dangerous command. Claude Code only today; see [`docs/cli-reference.md`](docs/cli-reference.md) §11 for exactly why Cursor and Codex aren't covered yet.
- `damping mcp wrap` — the same policy engine and audit log, for MCP tool calls too, not just your terminal. MCP calls that reach a server over HTTP/SSE (where a stdio proxy structurally can't sit in front) are covered through the agent's own pre-tool-use hook instead, judged by the same rules against the same audit trail.
- **Embeddable in a desktop app or IDE** — `damping hook pretooluse --hook-json` answers with the agent SDK's own hook-response JSON and hands a prompt-tier decision to your app's permission dialog as `"ask"`, instead of hard-denying because there's no terminal to ask at. Deny-tier rules still hard-deny, an allow never auto-approves on your behalf, and `--actor <label>` keeps your app's interceptions distinguishable in the audit trail it shares with the user's own CLI agents. See [`docs/cli-reference.md`](docs/cli-reference.md) §11.1.
...
- 351 BDD (Gherkin) scenarios, all wired to real code and passing, not just documentation — including a permanent regression suite for every command-wrapper, interpreter-`-c`, pipeline-stage, compound-command, and variable-expansion (`$HOME`) bypass ever found against this rule set. ("All wired" is re-verified, not assumed: a 2026-07 audit found 9 scenarios whose quote-bearing command text had silently never matched a step definition — reported green while never executing — fixed alongside the $HOME expansion.)
