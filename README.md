...
- A local `damping dashboard` (screenshot above) and `damping log` for replaying the full audit trail across both channels — with a risk-over-time chart, a top-triggered-rules breakdown, a time-range/rule-id/keyword filter, and a "load older" cursor for paging back through history, both computed over the full audit history, not just the visible table. The dashboard's UI (not the audit data itself) also switches between English and 繁體中文, and clicking the rule-count summary opens a plain-language explainer of what every active rule actually catches.
